...
def write_graph_to_directory(path, threshold, graphs, format):
    ...
    for key, value in graphs.iteritems():
        ...
        animal_id, state = key.split("-")
        state, _ = state.split(".")
        #file_name = str(animal_id) + "-" + state + "-" + trial + ".xml"
        file_name = str(animal_id) + "-" + state + ".xml"
        if not format:
...
def read_graphs_from_directory(path, threshold, animal_id, state, format):
    ...
    for filename in os.listdir(new_file_path):
        graph = nx.read_graphml(new_file_path + "/" + filename)
...
def create_tree_directories(path, threshold, nr_animals, format):
    ...
    ex: 0: 0.5 (threshold) -> 1 (id animal) -> Abstinence, Control, EtOH, Naltrexone(state)
        1: "mean" (threshold) -> Abstinence, Control, EtOH, Naltrexone(state)
...
def persist_statistics(graphs, path_statistics):
    df = pd.DataFrame(columns=["name", "nr_pos_edges", "nr_neg_edges", "sum_pos_edges",
                               "sum_neg_edges", "mean_pos_edges", "mean_neg_edges",
                               "nr_connected_components"])
    for graph_name, graph in graphs.iteritems():
        print(graph_name)
        fn_G = FunctionalNetwork(graph)
        graph_data = fn_G.create_statistics(graph_name)
        df = df.append(graph_data, ignore_index=True)

    df.to_csv(path_statistics, index=False)


def persist_edge_weight_distribution(graphs, path_edge_weight_distribution):

    for graph_name, graph in graphs.iteritems():
        print(graph_name)

        path_name = path_edge_weight_distribution + "/" + graph_name + ".png"
        fn = FunctionalNetwork(graph)
        fn.draw_edge_weight_distribution(path_name)
