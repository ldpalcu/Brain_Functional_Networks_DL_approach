...
cmd_opt.add_argument('-data', default="GRAPHSUM70", help='data folder name')
...
class GNNGraph(object):
    def __init__(self, g, label, node_tags=None, node_features=None):
        ...
        '''
        self.num_nodes = len(node_tags) if node_tags else 0
...
def load_data():
    ...
    with open('data/%s/%s.txt' % (cmd_args.data, cmd_args.data),
              'r') as f, open("data/%s/edges.txt" % cmd_args.data, "r") as e:
        n_g = int(f.readline().strip())
        for i in range(n_g):
            if cmd_args.edge_feat_dim > 0:
                edge_dict = {}
                nr_edges = int(e.readline().strip())
                for idx in range(nr_edges):
                    edge_u, edge_v, weight = e.readline().strip().split()
                    edge_dict.update({(int(edge_u), int(edge_v)): float(weight)})
                    edge_dict.update({(int(edge_v), int(edge_u)): float(weight)})
            ...
            for j in range(n):
                ...
                for k in range(2, len(row)):
                    if cmd_args.edge_feat_dim > 0:
                        weight = edge_dict[(j, row[k])]
                        weight_np = np.ndarray((1, ), dtype=float, buffer=np.array(weight))
                        g.add_edge(j, row[k], features=weight_np)
                    else:
                        g.add_edge(j, row[k])
